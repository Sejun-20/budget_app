...
function ReceiptIcon() {
  return (
    <svg width={ICON_SIZE} height={ICON_SIZE} viewBox="0 0 22 22" fill="none" stroke="#fff" strokeWidth="1.8">
...
const TILES = [
  { to: "/receipts/upload", label: "영수증", tint: "var(--color-primary-tint)", badge: "var(--color-primary-soft)", icon: <ReceiptIcon /> },
  ...
  { to: "/settings", label: "설정", tint: "var(--color-gray-tint)", badge: "var(--color-gray)", icon: <GearIcon /> },
];
        ...
        <div className="grid grid-cols-2 gap-5">
          {TILES.map((t) => (
            <Link
              key={t.label}
              to={t.to}
              className="flex h-[136px] flex-col items-center justify-center gap-3 rounded-[20px] px-[14px] active:opacity-80"
              style={{ background: t.tint }}
            >
              <div
                className="flex h-[56px] w-[56px] shrink-0 items-center justify-center rounded-[16px]"
                style={{ background: t.badge }}
              >
                {t.icon}
              </div>
              <span className="text-center text-sm font-semibold" style={{ color: "var(--color-text)" }}>
                {t.label}
              </span>
            </Link>
          ))}
